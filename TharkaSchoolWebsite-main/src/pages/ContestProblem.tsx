...
int main() {
  ...
  const { data: problem, isLoading, error } = useQuery<any>({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/contests/${contestId}/problems/${problemId}`, {
        headers: {
  ...
  useEffect(() => {
    if (error) {
      toast.error("Failed to load problem. You may not have access to this contest.");
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      const token = await getToken();
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/contests/${contestId}/problems/${problemId}/submissions`, {
  ...
  const handleRun = async () => {
    ...
    try {
      const token = await getToken();
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/contests/${contestId}/problems/${problemId}/submit`, {
        method: "POST",
        headers: {
    ...
    <div className="flex flex-col h-screen bg-background overflow-hidden">
      ...
      <div className="h-14 border-b bg-card flex items-center justify-between px-4 flex-shrink-0">
         <div className="flex items-center gap-4">
            <Link to={`/contest/${contestId}`} className="flex items-center gap-1 text-muted-foreground hover:text-foreground transition-colors text-sm font-medium">
            ...
            <div className="flex items-center gap-2">
               ...
               <Badge variant={problem.difficulty === "Easy" ? "secondary" : problem.difficulty === "Medium" ? "default" : "destructive"} className="text-[10px] px-1.5 py-0 h-5">
                   {problem.difficulty}
               </Badge>
            </div>
         </div>
